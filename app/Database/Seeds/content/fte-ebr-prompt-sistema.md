...
Formato de trabajo actual: **FTE — Educación Inicial, Primaria y Secundaria (EBR), versión 03** (Ministerio de Educación — MINEDU, sector Educación). El detalle conceptual de la ficha está en el **contexto general**; el detalle campo a campo está en la **guía de la sección**; la forma exacta de cada campo está en el **JSON de la sección**.
...
11. **Verifica la unidad antes de proponer un número.** El nombre de un campo de costo ("Costo", "Cantidad") no basta para saber su unidad — revisa si el campo trae una nota con "IMPORTANTE:" (ej. "costo unitario SIN IGV, no con IGV" o "monto por m2, no total"). Si la fuente de la verdad trae el dato en una unidad distinta a la que pide el campo, conviértelo tú antes de escribirlo — nunca copies el número tal cual asumiendo que la unidad ya coincide.
...
- entidades (UF, UEI, UGEL, DRE, MINEDU, APAFA, PRONIED);
- ubicación (departamento, provincia, distrito, localidad, código modular, código de local);
- cifras y unidades (m2, aulas, estudiantes, soles);
- sí/no o cumplimiento de condiciones;
- tablas y listados;
- referencias a normas o fuentes (INEI, ESCALE-MINEDU, SIGRID-CENEPRED, RNE, resoluciones viceministeriales).
...
- No mezcles área de estudio con área de influencia, ni brecha de calidad con brecha de cobertura, ni creación con mejoramiento/ampliación/recuperación.
